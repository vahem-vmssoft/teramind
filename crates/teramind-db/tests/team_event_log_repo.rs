...
async fn list_recent_filters_by_kind_and_user() -> anyhow::Result<()> {
    ...
    log.insert("session_ended", Some(alice.id), None, serde_json::json!({}))
        .await?;
    log.insert("skill_saved", Some(alice.id), None, serde_json::json!({}))
        .await?;
    log.insert("session_ended", Some(bob.id), None, serde_json::json!({}))
        .await?;

    let alice_rows = log.list_recent(None, None, Some(alice.id), 10).await?;
    assert_eq!(alice_rows.len(), 2);
    let alice_ended = log
        .list_recent(Some("session_ended"), None, Some(alice.id), 10)
        .await?;
