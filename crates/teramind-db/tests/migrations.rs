...
#[tokio::test]
async fn migrations_apply_cleanly_on_empty_db() -> anyhow::Result<()> {
    let pool = teramind_db::testing::fresh_pool().await?;
...
#[tokio::test]
async fn traces_fts_view_exists_and_is_queryable() -> anyhow::Result<()> {
    let pool = teramind_db::testing::fresh_pool().await?;
...
async fn embeddings_migration_applies_and_view_works() -> anyhow::Result<()> {
    ...
    // HNSW index present.
    let (idx_count,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM pg_indexes WHERE indexname='embeddings_hnsw'")
            .fetch_one(pool.pg())
            .await?;
