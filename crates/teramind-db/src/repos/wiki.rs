...
impl WikiRepo {
    ...
    pub async fn fetch_sessions_to_summarize(
        &self,
        model: &str,
        limit: u32,
    ) -> Result<Vec<SessionToSummarize>> {
        let rows: Vec<(Uuid, String, OffsetDateTime, OffsetDateTime, Option<String>)> =
            sqlx::query_as(
                r#"
        ...
        Ok(rows
            .into_iter()
            .map(
                |(sid, cwd, started_at, ended_at, end_reason)| SessionToSummarize {
    ...
    pub async fn load_snapshot(
        &self,
        session_id: SessionId,
    ) -> Result<Option<teramind_core::summarize::SessionSnapshot>> {
        // Session metadata.
        ...
        let turns = turn_rows
            .into_iter()
            .map(
                |(id, ord, up, at, th, sa)| teramind_core::summarize::TurnRow {
        ...
        // Tool calls.
        let tc_rows: Vec<(Uuid, Uuid, String, serde_json::Value, Option<String>, bool)> =
            sqlx::query_as(
                r#"
            ...
            .fetch_all(self.pool.pg())
            .await?;
        let tool_calls = tc_rows
            .into_iter()
            .map(
                |(id, tid, name, input, output, is_error)| teramind_core::summarize::ToolCallRow {
        ...
        .fetch_all(self.pool.pg())
        .await?;
        let file_diffs = fd_rows
            .into_iter()
            .map(|(tid, rel, lang, attr, diff, pre, post)| {
