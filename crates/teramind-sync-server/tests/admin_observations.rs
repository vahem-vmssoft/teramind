...
async fn list_filters_by_kind_and_status() -> anyhow::Result<()> {
    ...
    obs_repo.upsert("tool_chain",  "sig-tc",  &[s1], serde_json::json!({})).await?;
    obs_repo.upsert("problem_fix", "sig-fw",  &[s2], serde_json::json!({})).await?;
...
async fn list_applies_min_freq_filter() -> anyhow::Result<()> {
    ...
    let s1 = SessionId::new();
    let s2 = SessionId::new();
    obs_repo.upsert("tool_chain", "low-freq-sig",  &[s1], serde_json::json!({})).await?;
    obs_repo.upsert("tool_chain", "low-freq-sig",  &[s2], serde_json::json!({})).await?;

    // Frequency 5: insert 5 distinct sessions.
    let sessions5: Vec<SessionId> = (0..5).map(|_| SessionId::new()).collect();
    for s in &sessions5 {
        obs_repo.upsert("tool_chain", "high-freq-sig", &[*s], serde_json::json!({})).await?;
    }
    ...
    let body: serde_json::Value = r.json().await?;
    let obs = body["observations"].as_array().unwrap();
    assert_eq!(obs.len(), 1, "only high-freq obs should pass min_freq=3 filter");
...
async fn show_returns_context_blob() -> anyhow::Result<()> {
    ...
    obs_repo.upsert("tool_chain", "show-sig", &[s], ctx.clone()).await?;
    let obs = obs_repo.find_by_sig("tool_chain", "show-sig").await?.expect("must exist");
