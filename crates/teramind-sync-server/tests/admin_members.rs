...
async fn lists_members_with_device_counts() -> anyhow::Result<()> {
    ...
    let user = state.users.upsert_by_email("alice@example.com", None).await?;
    state.devices.insert(user.id, "device-1", b"tok1hash_________", b"pubkey1").await?;
    state.devices.insert(user.id, "device-2", b"tok2hash_________", b"pubkey2").await?;
...
async fn creates_invite_and_returns_code_once() -> anyhow::Result<()> {
    ...
    assert!(code.starts_with("TM-"), "code should start with TM-, got: {code}");
    ...
    assert!(invites[0].get("code").is_none(), "raw code must not appear in list response");
...
async fn revokes_device_via_admin_endpoint() -> anyhow::Result<()> {
    ...
    // Seed user + device.
    let user = state.users.upsert_by_email("carol@example.com", None).await?;
    let device = state.devices.insert(user.id, "carol-laptop", b"tok3hash_________", b"pubkey3").await?;
    ...
    let r = reqwest::Client::new()
        .post(format!("http://{addr}/admin/devices/{}/revoke", device.id.0))
        .header("Cookie", &cookie).send().await?;
    assert_eq!(r.status(), 200);
    let body: serde_json::Value = r.json().await?;
    ...
    assert!(post.is_empty(), "device should be revoked and absent from active list");
