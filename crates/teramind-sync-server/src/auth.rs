...
pub async fn auth_middleware(
    ...
    let proof = parts
        .headers
        .get("x-teramind-proof")
        .and_then(|v| v.to_str().ok())
        .ok_or(StatusCode::FORBIDDEN)?
        .to_string();
    let body_bytes = axum::body::to_bytes(body, state.cfg.ingest.max_request_body_bytes)
        .await
    ...
    // 5. Fire-and-forget last-seen update.
    {
        ...
        tokio::spawn(async move {
            let _ = devices.touch_last_seen(did).await;
        });
    }
