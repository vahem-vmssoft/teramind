...
pub async fn login(
    ...
    let parsed = PasswordHash::new(&admin_cfg.admin_password_hash).map_err(|_| {
        DashboardError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            "internal",
            "bad password hash in config",
        )
    })?;
    if Argon2::default()
    ...
    let session = AdminSession {
        ...
        expires_at: OffsetDateTime::now_utc()
            + Duration::hours(admin_cfg.admin_session_ttl_hours as i64),
    };
    ...
    headers.insert(
        header::SET_COOKIE,
        format!("tmd_admin={token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age={max_age}")
            .parse()
            .unwrap(),
    );
    Ok((
...
pub async fn logout() -> impl IntoResponse {
    ...
    headers.insert(
        header::SET_COOKIE,
        "tmd_admin=; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=0"
            .parse()
            .unwrap(),
    );
