#![cfg(unix)]
//! team-sync §2.1 — `teramind doctor` extended with team-mode health surfaces.
//! Without team.toml present, the doctor output must carry the negative-status
//! "team mode:   not configured" line (pin the contract that already lives in
//! doctor.rs near line 104).
...
#[test]
fn doctor_reports_team_mode_not_configured_without_team_toml() {
    ...
    // Sanity: no team.toml under XDG_CONFIG_HOME/teramind.
    let team_toml = tmp
        .path()
        .join("xdg-config")
        .join("teramind")
    ...
    assert!(
        !team_toml.exists(),
        "test setup wrote a team.toml unexpectedly"
    );
    ...
    let has_not_configured = stdout
        .lines()
        .any(|l| l.contains("team mode:") && l.contains("not configured"));
    ...
    assert!(
        has_not_configured,
        "doctor stdout missing 'team mode: not configured':\n{stdout}"
    );
}
