...
fn claude_install_uninstall_roundtrip() {
    ...
    let env: Vec<(&str, String)> = vec![
        (
            "CLAUDE_HOME",
            claude_home.path().to_string_lossy().into_owned(),
        ),
        (
            "TERAMIND_PLUGIN_TEMPLATE_DIR",
    ...
    assert!(
        !body.contains("@TERAMIND_PLUGIN_DIR@"),
        "placeholder left unpatched in manifest"
    );
    assert!(
    ...
    let hook_script = claude_home
        .path()
        .join("plugins/teramind/hooks/session_start.sh");
    let body = std::fs::read_to_string(&hook_script).unwrap();
    assert!(
        !body.contains("@TERAMIND_HOOK_BIN@"),
        "placeholder left unpatched in hook script"
    );
    ...
    assert!(
        !body.contains("@TERAMIND_MCP_BIN@"),
        "MCP placeholder left unpatched"
    );
    ...
    assert!(
        !claude_home.path().join("plugins/teramind").exists(),
        "plugin dir not removed"
    );
}
