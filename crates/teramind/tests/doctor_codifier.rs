#![cfg(unix)]
//! codifier §10 — `teramind doctor` surfaces codifier health lines covering
//! enabled/disabled and observation/candidate counts. Daemon up; exec
//! `doctor`; pin the codifier surfaces. Counts are sourced from the daemon
//! via StatusReport (cheap COUNT queries against skill_observations /
//! skill_candidates).
...
#[test]
fn doctor_surfaces_codifier_status_and_counts() {
    ...
    let has_codify_line = stdout
        .lines()
        .any(|l| l.contains("codifier") || l.contains("codify"));
    let has_count_line = stdout
        .lines()
        .any(|l| l.contains("observations=") || l.contains("candidates pending="));
    ...
    assert!(
        has_codify_line,
        "doctor stdout missing codifier/codify line:\n{stdout}"
    );
    assert!(
        has_count_line,
        "doctor stdout missing observations/candidates count line:\n{stdout}"
    );
}
