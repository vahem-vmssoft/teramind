#![cfg(unix)]
//! pgvector §10 — `teramind doctor` surfaces embedding provider name/model,
//! health, and backlog. Daemon up; exec `doctor`; pin the embedding surfaces.
...
#[test]
fn doctor_surfaces_embedding_provider_and_backlog() {
    ...
    let has_embed_line = stdout
        .lines()
        .any(|l| l.contains("embedding") || l.contains("embed"));
    ...
    assert!(
        has_embed_line,
        "doctor stdout missing embedding/embed line:\n{stdout}"
    );
    assert!(
        has_backlog_line,
