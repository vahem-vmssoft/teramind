...
// Shutdown must both reply Ok and actually fire the notify the daemon's main
// loop awaits — without the notify, `teramind stop` is a silent no-op.
#[tokio::test]
async fn shutdown_request_fires_notify() {
    ...
    let stream = connect(&sock).await.unwrap();
    let mut client = StreamClient::new(stream);
    let r = client.request(Request::Shutdown).await.unwrap();
    assert!(matches!(r, Response::Ok), "unexpected: {r:?}");

    // The notify must already be pending; resolve quickly or fail.
    tokio::time::timeout(std::time::Duration::from_secs(2), shutdown.notified())
        .await
        .expect("shutdown notify never fired");
}
