...
async fn ollama_e2e_paraphrase_lookup() -> anyhow::Result<()> {
    ...
    for _ in 0..40 {
        ...
        if repo.backlog(&model).await? == 0 {
            break;
        }
    }
    assert_eq!(
        repo.backlog(&model).await?,
        0,
        "worker should drain backlog"
    );

    let q_emb = provider
        .embed(&["how does the JWT refresh flow work".to_string()])
        .await
    ...
    assert!(
        !hits.is_empty(),
        "semantic search should return the paraphrased turn"
    );
    assert!(
        hits[0].semantic_score > 0.3,
        "expected non-trivial cosine similarity, got {}",
