//! Integration test: `codify` MCP tool dispatches
//! `Request::CodifyNow` to the daemon over IPC.
#![cfg(unix)]
...
/// Minimal mock IPC server that records all requests and replies `CodifyQueued`.
#[derive(Clone)]
...
impl IpcServer for RecordingHandler {
    async fn handle_request(&self, req: Request) -> Response {
        ...
        Response::CodifyQueued {
            observation_id: "test-obs-id-001".to_string(),
        }
    }
...
fn drive_mcp_subprocess(sock_path: std::path::PathBuf) -> String {
    ...
    // 3. tools/call codify
    let call = r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"codify","arguments":{"seed_session_ids":["sess-abc-123"],"hint":"always use clippy before commit"}}}"#;
...
async fn codify_dispatches_ipc_request() {
    ...
    // Verify the mock server received a CodifyNow request.
    ...
    let found = requests.iter().any(|r| {
        matches!(
            ...
            if seed_session_ids == &["sess-abc-123"] && hint == "always use clippy before commit"
        )
    });
    assert!(
        found,
        "expected CodifyNow(seed_session_ids=[sess-abc-123], hint=...); got: {requests:?}"
    );
}
