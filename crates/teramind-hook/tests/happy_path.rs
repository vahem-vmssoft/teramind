...
async fn hook_tool_call_lifecycle_persists() {
    ...
    let payloads = vec![
        format!(
            r#"{{"hook_event_name":"SessionStart","session_id":"{sid}","cwd":"/w","source":"startup"}}"#
        ),
        format!(
            r#"{{"hook_event_name":"UserPromptSubmit","session_id":"{sid}","cwd":"/w","prompt":"do it"}}"#
        ),
        format!(
            r#"{{"hook_event_name":"PreToolUse","session_id":"{sid}","cwd":"/w","tool_name":"Edit","tool_input":{{"file_path":"/w/x.rs"}}}}"#
        ),
        format!(
            r#"{{"hook_event_name":"PostToolUse","session_id":"{sid}","cwd":"/w","tool_name":"Edit","tool_input":{{"file_path":"/w/x.rs"}},"tool_response":"ok"}}"#
        ),
    ];
...
async fn hook_session_start_emits_auto_recall_digest() {
    ...
    let prior_turn = trace
        .upsert_turn(prior_sid, 0, now, Some("yesterday's bug fix"))
        .await
        .unwrap();
    trace
        .finalize_turn(
            prior_turn,
            now,
            Some("fixed by adjusting timeout"),
            None,
            None,
            None,
            None,
