...
pub fn run() -> Result<(), String> {
    ...
    let parsed: HookInput = serde_json::from_str(raw).map_err(|e| format!("parse failed: {e}"))?;
    let env = translate::translate(parsed).ok_or_else(|| "translate returned None".to_string())?;
